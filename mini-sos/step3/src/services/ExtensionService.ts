...
export class ExtensionService {
  ...
  registerExtension(manifest: Extension) {
    ...
    // TODO: matchesNamedTag 可以简化下
    container
      .bind(Symbol.for(serviceId))
      .toDynamicValue(() => manifest)
      .when((request: any) => request.target.matchesNamedTag(fullName));
  }

  // 这个和注册扩展是一回事, 只不过 serviceId 不一样而已
  registerExtensionImpl(serviceId: string, manifest: Extension) {
  ...
  registerExtensionPoint(serviceId: string) {
