...
function updateClassComponent(fiber) {
  ...
  fiber.partialState = null; // 根据新的 state 和 props 生成新的组件返回值（children）

  const newChildElements = instance.render(); // 为每一个 child 创建一个 fiber 并且和链表到 wipFiber
...
function performUnitOfWork(fiber) {
  beginWork(fiber); // 返回下一个 fiber(next unit of work)
  ...
  if (fiber.child) {
    ...
    while (nextFiber) {
      ...
      if (nextFiber.sibling) {
        ...
        // uncle
        nextFiber = nextFiber.parent; // 继续 loop parent.sibling
      }
    }
  }
}
...
const commitWork = fiber => {
  if (!fiber) return;
  let parentFiber = fiber.parent; // 如果是组件的话，那么得 traverse parent 直到找到 dom 元素
...
Component.prototype.isReactComponent = {}; // 实例化组件
